    ...
    const fetchMachineData = async () => {
      try {
        ...
        if (data.machines) {
          ...
          const connectedMachine = data.machines.find(
            (m: MachineDetails) => m.pipeline_status === 'connected'
          );
  ...
  const fetchSensorData = async (machineId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/pipeline/sensor-data?machineId=${machineId}&limit=10`
      );
      const data = await response.json();
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="text-right">
                ...
                <p className="text-lg font-semibold text-green-600">
                  {
                    machines.filter(m => m.pipeline_status === 'connected')
                      .length
                  }
                </p>
              </div>
                    ...
                    {machines.map(machine => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div
                              className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(machine.pipeline_status)}`}
                            >
                              {getStatusIcon(machine.pipeline_status)}
                              <span className="ml-1 capitalize">
                                {machine.pipeline_status}
                              </span>
                            </div>
            ...
            {selectedMachine ? (
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div
                      className={`inline-flex items-center px-3 py-1 text-sm font-medium rounded-full ${getStatusColor(selectedMachine.pipeline_status)}`}
                    >
                      {getStatusIcon(selectedMachine.pipeline_status)}
                      <span className="ml-2 capitalize">
                        {selectedMachine.pipeline_status}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                      ...
                      <p className="text-2xl font-bold text-green-900 dark:text-green-100">
                        {selectedMachine.total_messages_received?.toLocaleString() ||
                          0}
                      </p>
                    </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-8">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    Choose a machine from the list to view detailed information
