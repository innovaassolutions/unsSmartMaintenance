...
export default function MaintenanceDashboard() {
      ...
      try {
        ...
        const equipmentHealth: EquipmentHealth[] = (
          ...
          return {
            ...
            machineName: machine.display_name as string,
            healthScore,
            status: getStatus(healthScore),
            lastMaintenance: new Date(
              Date.now() - (7 + Math.random() * 14) * 24 * 60 * 60 * 1000
            ).toISOString(),
            nextMaintenance: new Date(
              Date.now() + (3 + Math.random() * 7) * 24 * 60 * 60 * 1000
            ).toISOString(),
  ...
  const getAlertColor = (severity: string) => {
    switch (severity) {
      case 'critical':
        return 'text-red-600 bg-red-100 dark:bg-red-900/20 dark:text-red-200 border-red-200';
      case 'high':
        return 'text-orange-600 bg-orange-100 dark:bg-orange-900/20 dark:text-orange-200 border-orange-200';
      case 'medium':
        return 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-200 border-yellow-200';
      case 'low':
        return 'text-blue-600 bg-blue-100 dark:bg-blue-900/20 dark:text-blue-200 border-blue-200';
      default:
        return 'text-gray-600 bg-gray-100 dark:bg-gray-900/20 dark:text-gray-200 border-gray-200';
    }
  };
  ...
  const getTaskStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return 'text-gray-600 bg-gray-100 dark:bg-gray-900/20 dark:text-gray-200';
      case 'overdue':
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    Equipment health diagnostics and maintenance schedules
                  </p>
                </div>
              </div>
            </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm animate-pulse"
                >
                  ...
                  <p className="text-xs opacity-75">
                    ...
                    {alert.estimatedFailureTime && (
                      <span className="block">
                        Est. failure:{' '}
                        {new Date(alert.estimatedFailureTime).toLocaleString()}
                      </span>
                    )}
                  </p>
                </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div
                              className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full mt-1 ${getHealthColor(machine.status)}`}
                            >
                    ...
                    {tasks.map(task => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            <div
                              className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${getTaskStatusColor(task.status)}`}
                            >
