    ...
    const fetchProductionData = async () => {
      try {
        ...
        const machineStatuses: MachineStatus[] = (
          ...
          name: machine.display_name as string,
          status:
            (machine.pipeline_status as string) === 'connected'
              ? 'running'
              : 'offline',
        ...
        const completedJobs = simulatedJobs.filter(
          job => job.status === 'completed'
        );
        const runningJobs = simulatedJobs.filter(
          job => job.status === 'running'
        );
        const totalActual =
          completedJobs.reduce((sum, job) => sum + job.partCount, 0) +
          runningJobs.reduce((sum, job) => sum + job.partCount, 0);
        const totalTarget = simulatedJobs.reduce(
          (sum, job) => sum + job.targetCount,
          0
        );
                    ...
                    {jobs.map(job => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-3">
                            <div
                              className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${getJobStatusColor(job.status)}`}
                            >
                            ...
                            <div
                              className={`px-2 py-1 text-xs font-medium rounded ${getPriorityColor(job.priority)}`}
                            >
