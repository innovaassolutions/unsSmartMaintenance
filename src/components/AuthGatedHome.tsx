...
function AuthGatedHomeInner() {
  ...
  // Hide the SSR landing content when authenticated user sees dashboard
  useEffect(() => {
    if (mounted && !showLanding) {
      const ssrLanding = document.getElementById('landing-ssr');
      if (ssrLanding) ssrLanding.style.display = 'none';
    }
  }, [mounted, showLanding]);
